...
export default function TextForm(props) {
    ...
    <div className={`container text-${props.mode==='light'?'dark':'light'}`}>
        <h1>{props.heading}</h1>
        <div className="mb-3">
            <textarea className={`form-control`} style={{backgroundColor: props.mode==='dark' ? 'grey':'white',color: props.mode==='dark' ? 'white':'black'}} id="exampleFormControlTextarea1"  onChange={onChange} rows="8" value={text}></textarea>
        </div>
        <button className="btn btn-primary mx-2" onClick={onUpclick}>UPPERCASE</button>
        <button className="btn btn-primary mx-2" onClick={onlowclick}>lowercase</button>
        <button className="btn btn-primary mx-2" onClick={onsenclick}>Sentance case</button>
